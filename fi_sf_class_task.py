...
class StateFarm_Classification:
    ...
    def step_1(self): # clean and prepare data
        ...
        # feature importance
        #vars = self.trainXY.drop(columns=['y']).columns
        #mut_info_score = mutual_info_classif(self.trainX,self.trainY)
        #rf = RandomForestClassifier()
        #rf.fit(self.trainX,self.trainY)
        #importances = rf.feature_importances_
        #fi_df = pd.DataFrame(np.array([mut_info_score,importances]).T, columns=['mut_info','MDI'], index=vars)
        import matplotlib.pyplot as plt
        self.trainXY[['x7','x4']].hist()
        #fi_df['mut_info'].sort_values(ascending=False).iloc[:20].plot.bar()
        #plt.show()
        #plt.close()
        #fi_df['MDI'].sort_values(ascending=False).iloc[:20].plot.bar()
        plt.show()
